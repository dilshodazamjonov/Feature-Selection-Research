...
from credit_risk_fs.evaluation._feature_utils import (
...
def process_fold(
    ...
    run_dir = Path(features_dir).parent
    llm_response_dir = run_dir / "llm_responses" / f"fold_{fold}"
    feature_rankings_dir = run_dir / "feature_rankings"
    selected_feature_sets_dir = run_dir / "selected_feature_sets"
    ...
    if selector is not None and hasattr(selector, "set_artifact_dir"):
        selector.set_artifact_dir(llm_response_dir)
