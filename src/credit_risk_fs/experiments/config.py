...
def load_config_stack(paths: list[str | Path]) -> dict[str, Any]:
    config = copy.deepcopy(DEFAULT_CONFIG)
    for path in paths:
        config = _merge_dicts(config, load_project_config(path))
    return config


def dataset_config_path(dataset_name: str) -> Path:
    return Path("configs") / "datasets" / f"{dataset_name.lower()}.yaml"


def experiment_config_path(dataset_name: str, experiment_name: str = "matrix") -> Path:
    return Path("configs") / "experiments" / f"{dataset_name.lower()}_{experiment_name.lower()}.yaml"


def load_named_project_config(
    dataset_name: str,
    *,
    experiment_name: str | None = None,
) -> dict[str, Any]:
    paths: list[str | Path] = [Path("configs") / "base.yaml", dataset_config_path(dataset_name)]
    if experiment_name is not None:
        paths.append(experiment_config_path(dataset_name, experiment_name))
    return load_config_stack(paths)
...
def apply_feature_budget_to_selector_kwargs(
    ...
    elif name in {"clip", "clip_then_mrmr", "clip_v2", "clip_v2_then_mrmr"}:
        updated["feature_budget"] = feature_budget
