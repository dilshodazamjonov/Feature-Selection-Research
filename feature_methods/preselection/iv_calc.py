...
class IVFilter:
    def __init__(
        ...
        output_dir: Optional[str] = None,   # <-- controlled externally
    ):
        """
        Params:
    ...
    # Feature processing
    # -------------------------
    def _process_numeric_feature(self, series, y):
        try:
        ...
        if self.min_bin_pct is not None:
            ...
            stats["_small_bin_flag"] = ((stats["count"] / total) < self.min_bin_pct).astype(int)
    ...
    def _process_categorical_feature(self, series, y):
