...
def _workload() -> dict:
    return {
        ...
        "effective_wall_clock_limit_seconds": 21600.0,
        "composition_rule": (
...
def test_component_cost_composition_never_downgrades_heavy_work():
    ...
    assert full_catboost.effective_wall_clock_limit_seconds == 21600
...
def test_execution_and_manifest_use_same_effective_classification_and_timeout(
    ...
    assert request.max_wall_clock_seconds == 21600
    assert request.manifest_metadata["effective_wall_clock_limit_seconds"] == 21600
