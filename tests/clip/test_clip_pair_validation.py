...
def test_duplicate_text_views_fail_validation(legacy_artifact_path):
    text = pd.read_parquet(
        legacy_artifact_path("results/clip/text_baseline/homecredit_text_embeddings.parquet")
    )
    stat = pd.read_parquet(
        legacy_artifact_path("results/clip_v2/statistical_view/homecredit_statistical_vectors.parquet")
    )
...
def test_missing_statistical_view_fails_validation(legacy_artifact_path):
    text = pd.read_parquet(
        legacy_artifact_path("results/clip/text_baseline/homecredit_text_embeddings.parquet")
    )
    stat = pd.read_parquet(
...
def test_stale_source_hash_fails_manifest_boundary(
    tmp_path, legacy_artifact_path, legacy_config_paths
):
    config = legacy_config_paths(load_contrastive_data_config(
        "configs/corrected_homecredit_clip/contrastive_data.yaml"
    ))
    manifest = json.loads(legacy_artifact_path("results/clip/dry_run/training_manifest.json").read_text(encoding="utf-8"))
    source_hashes = json.loads(legacy_artifact_path("results/clip/dry_run/source_hashes.json").read_text(encoding="utf-8"))
