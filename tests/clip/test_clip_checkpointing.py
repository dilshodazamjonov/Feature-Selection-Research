...
def test_selected_checkpoint_loads_with_current_upstream_hashes(
    legacy_artifact_path, legacy_config_paths
):
    config = legacy_config_paths(
        load_training_config("configs/corrected_homecredit_clip/training.yaml")
    )
    ...
    model = load_checkpoint(
        checkpoint_path=legacy_artifact_path(selection["selected_checkpoint_path"]),
        manifest_path=legacy_artifact_path(
