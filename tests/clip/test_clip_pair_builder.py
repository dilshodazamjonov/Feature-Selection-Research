...
def test_contrastive_dry_run_does_not_overwrite_full_pair_artifacts(tmp_path):
...
def test_contrastive_dry_run_keeps_actual_full_run_hashes_unchanged():
    ...
    full_run_files = [
        ...
        config.output_dir / "lendingclub_v2_external_pairs.parquet",
        config.output_dir / "negative_exclusion_pairs.parquet",
        config.output_dir / "near_duplicate_text_audit.csv",
        config.output_dir / "contrastive_tensor_schema.json",
        config.output_dir / "contrastive_pair_manifest.json",
        config.output_dir / "split_manifest.json",
        config.output_dir / "negative_policy_manifest.json",
        config.output_dir / "negative_candidate_audit.csv",
        config.output_dir / "near_duplicate_threshold_sensitivity.csv",
        config.output_dir / "pair_quality_audit.csv",
        config.output_dir / "pair_quality_audit.json",
    ]
